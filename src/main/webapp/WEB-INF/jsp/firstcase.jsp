    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">装修档案馆</li>
            </ol>
        </nav>
    </div>
</div>
...
<section class="banner-bottom py-5">
    <div class="container container-narrow py-md-5">
        <h3 class="heading text-center mb-3 mb-sm-5">案例/case</h3>
        ...
        <div class="pages">
            <a href="<%=path%>/firstCase/${pageInfo.get("prePage")}">«</a>
            <c:if test="${pageInfo.get('prePage')}>2">
                <a href="<%=path%>/firstCase/${pageInfo.get("prePage")}">${pageInfo.get("prePage")}</a></c:if>
            <a href="<%=path%>/firstCase/${pageIndex}">${pageIndex}</a>
            <c:if test="${pageInfo.get('nextPage')}>2">
                <a href="<%=path%>/firstCase/${pageInfo.get("prePage")}">${pageInfo.get("prePage")}</a></c:if>
            <a href="<%=path%>/firstCase/${pageInfo.get("nextPage")}">»</a>
        </div>
    </div>
</section>
